...
(async () => {
  ...
  const inp = () => dbg().input;
  // first-person semantics: move stick is yaw-relative, aim stick TURNS you
  const setYaw = r => { dbg().app.yaw = r; };
  ...
  await sleep(60);
  ok(inp().my > 0.7 && inp().my < 0.95 && Math.abs(inp().mx) < 0.05, 'FP: tilt right strafes right (my=' + inp().my.toFixed(2) + ', mx=' + inp().mx.toFixed(2) + ')');
  ok(!inp().sprint, 'partial tilt does not sprint');
  c.touch('touchmove', $('tMove'), MC.x + 90, MC.y, 1);
  await sleep(60);
  ok(inp().my > 0.99, 'FP: clamped at the rim → my=' + inp().my.toFixed(2));
  ok(inp().sprint === true, 'full tilt auto-sprints');
  c.touch('touchmove', $('tMove'), MC.x, MC.y - 60, 1);
  await sleep(60);
  setYaw(0);
  ok(inp().mx > 0.9 && Math.abs(inp().my) < 0.05, 'FP: tilt up walks forward along yaw (mx=' + inp().mx.toFixed(2) + ')');
  ...
  const y0 = dbg().app.yaw;
  c.touch('touchmove', $('tAim'), AC.x + 40, AC.y, 2);
  await sleep(120);
  ok(dbg().app.yaw > y0 + 0.05, 'FP: aim right TURNS you right (yaw ' + y0.toFixed(2) + ' → ' + dbg().app.yaw.toFixed(2) + ')');
  ok(inp().fire === true, 'deflecting the aim stick auto-fires');
  const y1 = dbg().app.yaw;
  c.touch('touchmove', $('tAim'), AC.x - 30, AC.y + 30, 2);
  await sleep(120);
  ok(dbg().app.yaw < y1 - 0.05, 'FP: aim left turns you left (yaw ' + y1.toFixed(2) + ' → ' + dbg().app.yaw.toFixed(2) + ')');
  ok(Math.abs(Math.hypot(inp().aimx, inp().aimy) - 1) < 0.01, 'FP: aim is always the unit view vector (' + inp().aimx.toFixed(2) + ',' + inp().aimy.toFixed(2) + ')');
  ...
  setYaw(0);
  await sleep(60);
  ok(inp().mx > 0.8 && inp().fire && dbg().app.yaw > 0.02, 'FP: both thumbs tracked (walk forward ' + inp().mx.toFixed(2) + ' + turn ' + dbg().app.yaw.toFixed(2) + ' + fire)');
  c.touch('touchend', $('tMove'), MC.x, MC.y, 1);
  c.touch('touchend', $('tAim'), AC.x, AC.y, 2);
  await sleep(60);
  ...
  await sleep(60);
  ok(Math.hypot(f.dbg().input.mx, f.dbg().input.my) > 0.7, 'FP: touchstart+move drives the stick (mag=' + Math.hypot(f.dbg().input.mx, f.dbg().input.my).toFixed(2) + ')');
